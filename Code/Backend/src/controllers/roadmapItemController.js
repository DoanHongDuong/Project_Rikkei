...
const createItem = async (req, res) => {
    try {
        ...
        const { title, start_date, end_date, sort_order, status } = req.body;
        
        if (new Date(end_date) < new Date(start_date)) {
            return res.status(400).json({ success: false, message: 'end_date cannot be before start_date' });
        }

        const item = await roadmapItemService.createItem(roadmapId, { title, start_date, end_date, sort_order, status });
